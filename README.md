...
- **Reusable, team-ready Terraform** — a parameterized VPC **module** that stamps out multiple environments (dev/prod), with **remote state** stored in versioned S3.
...
└── terraform/
    ├── 01-vpc-foundation/     # the VPC network foundation, rebuilt in Terraform (flat)
    └── 02-vpc-module/         # reusable VPC module (dev + prod) + S3 remote state backend
```
...
| 04 | VPC foundation (Terraform) | Terraform | Providers, state, HCL, variables, full init/plan/apply/destroy loop | ✅ Complete |
| 05 | Reusable module + remote state | Terraform | Parameterized VPC module, dev/prod from one module, S3 remote state (versioned) | ✅ Complete |
...
- **No secrets or state in source** — credentials live only in `~/.aws/`; Terraform state lives in S3; `*.tfstate` and `.terraform/` are gitignored.
